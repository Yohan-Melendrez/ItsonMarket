/**
 * Inicializa el módulo de usuarios según la ruta actual.
 * Redirige a perfil propio, editar perfil o perfil de otro usuario.
...
/**
 * Carga y muestra el perfil de un usuario específico,
 * mostrando publicaciones, reseñas y estadísticas.
 * @async
 * @function initPerfilUsuario
 * @param {string} userId - ID del usuario que se desea cargar.
...
async function initPerfilUsuario(userId) {
  ...
  try {
    ...
    if (perfilAvatar) {
      ...
      perfilAvatar.onerror = () =>
        (perfilAvatar.src = "/imgs/default-avatar.svg");
    }
    if (perfilNombre) perfilNombre.textContent = usuario.nombre || "Usuario";
    if (perfilCorreo)
      perfilCorreo.textContent = usuario.correo_institucional || "";
    if (perfilCarreraBadge)
      perfilCarreraBadge.textContent = usuario.carrera || "Sin carrera";
    if (statReputacion) {
    ...
    if (infoCarrera)
      infoCarrera.textContent = getCarreraNombre(usuario.carrera) || "-";
    if (infoCorreo)
      infoCorreo.textContent = usuario.correo_institucional || "-";
    ...
    tabs.forEach((tab) => {
      tab.addEventListener("click", () => {
        ...
        if (tabPublicaciones)
          tabPublicaciones.classList.toggle(
            "hidden",
            tabName !== "publicaciones"
          );
        if (tabResenas)
          tabResenas.classList.toggle("hidden", tabName !== "resenas");
...
/**
 * Carga y renderiza las publicaciones de un usuario específico.
 * @async
 * @function cargarPublicacionesUsuario
 * @param {string} userId - ID del usuario propietario de las publicaciones.
 * @returns {Promise<void>}
 */
async function cargarPublicacionesUsuario(userId) {
  ...
  try {
    ...
    const publicaciones =
      data.items || data.publicaciones || (Array.isArray(data) ? data : []);
      ...
      .map(
        (pub) => `
            <a href="#/publicaciones/${
              pub._id
            }" class="card publicacion-mini-card">
                <div class="publicacion-mini-content">
                    <img src="${
                      pub.detalles?.imagenes?.[0] || "/imgs/default-product.svg"
                    }" 
                         alt="${pub.titulo}" 
                         class="publicacion-mini-imagen" />
                    <div class="publicacion-mini-info">
                        <h4>${pub.titulo}</h4>
                        <p class="publicacion-mini-precio">${formatCurrency(
                          pub.precio
                        )}</p>
                    </div>
                </div>
    ...
    container.innerHTML =
      '<p class="text-center text-muted">Error al cargar publicaciones</p>';
  }
}

/**
 * Inicializa la vista del perfil del usuario autenticado.
 * Carga datos personales, estadísticas, publicaciones y reseñas.
 * @async
 * @function initPerfil
 * @returns {Promise<void>}
 */
...
/**
 * Carga estadísticas del usuario: número de publicaciones y transacciones.
 * @async
 * @function cargarEstadisticas
 * @param {string} userId - ID del usuario para obtener estadísticas.
 * @returns {Promise<void>}
 */
async function cargarEstadisticas(userId) {
  ...
  try {
    ...
    const resPub = await fetch(`/api/publicaciones/vendedor/${userId}`, {
      headers,
    });
    ...
    const resTrans = await fetch(`/api/transacciones?vendedor_id=${userId}`, {
      headers,
    });
    ...
    if (statTransacciones) {
      const transacciones =
        transData.data ||
        transData.transacciones ||
        (Array.isArray(transData) ? transData : []);
...
/**
 * Carga y renderiza las publicaciones del usuario autenticado.
 * @async
...
async function cargarMisPublicaciones() {
  ...
  try {
    ...
    container.innerHTML =
      '<p class="empty-state-message">Error al cargar publicaciones</p>';
  }
}

/**
 * Carga y renderiza las reseñas recibidas por el usuario autenticado.
 * @async
...
async function cargarMisResenas() {
  ...
  try {
    ...
    const res = await fetch(`/api/transacciones?vendedor_id=${userId}`, {
      headers,
    });
    const payload = await res.json();
    const transacciones = Array.isArray(payload.data)
      ? payload.data
      : Array.isArray(payload)
      ? payload
      : [];

    const resenas = transacciones
        ...
        (r) => `
            <div class="card review-card">
                <div class="flex items-start gap-4">
                    <img src="${
                      r.autor?.foto || "/imgs/default-avatar.svg"
                    }" alt=""
                         class="review-avatar">
                    <div class="review-content">
                        <div class="flex items-center justify-between">
                            <h4 class="review-author">${
                              r.autor?.nombre || r.autor?.itson_id || "Usuario"
                            }</h4>
    ...
    container.innerHTML =
      '<p class="empty-state-message">Error al cargar reseñas</p>';
  }
}

/**
 * Inicializa la pantalla de edición de perfil:
 * previsualización de avatar, validación de contraseñas,
 * guardado de cambios y eliminar cuenta.
...
function initEditarPerfil() {
  ...
  if (avatarPreview)
    avatarPreview.src = usuario.foto || "/imgs/default-avatar.svg";
  ...
  if (form) {
    form.addEventListener("submit", async (e) => {
      ...
      const passConfirmar = document.getElementById(
        "editPasswordConfirmar"
      )?.value;
...
/**
 * Elimina una publicación propia y actualiza la lista.
 * @async
...
/**
 * Devuelve el nombre completo de una carrera a partir de su sigla.
 * @function getCarreraNombre
 * @param {string} codigo - Código de la carrera (ej. ISW, LAE).
 * @returns {string} Nombre completo de la carrera.
 */
...
/**
 * Expone la función initUsuarios globalmente.
 * @global
 */

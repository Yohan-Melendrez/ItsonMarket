...
/**
 * Inicializa la vista de transacciones cargando filtros, datos y el módulo de calificación.
...
/**
 * Inicializa los filtros de transacciones (tabs, estado, mes, limpiar filtros).
...
/**
 * Carga las transacciones del usuario autenticado desde el backend.
 * Muestra estados de carga, errores y actualiza el contador.
 * @async
...
async function cargarTransacciones() {
  ...
  try {
    ...
    transaccionesData = Array.isArray(data)
      ? data
      : data.data || data.transacciones || [];
...
/**
 * Renderiza la lista de transacciones aplicando filtros y orden por fecha.
...
function renderTransacciones() {
  ...
  filtered.sort(
    (a, b) =>
      new Date(b.fecha_transaccion || b.createdAt) -
      new Date(a.fecha_transaccion || a.createdAt)
  );
  ...
  lista.innerHTML = filtered
    .map((t) => crearTransaccionCard(t, userId, userItsonId))
    .join("");
}

/**
 * Genera la estructura HTML de una tarjeta de transacción para mostrar en la lista.
 * @function crearTransaccionCard
 * @param {Object} trans - Datos de la transacción.
 * @param {string} userId - ID del usuario autenticado.
 * @param {string} userItsonId - ITSON ID del usuario autenticado.
 * @returns {string} HTML generado para la tarjeta.
 */
function crearTransaccionCard(trans, userId, userItsonId) {
  const esCompra =
    trans.comprador_id?._id === userId ||
  ...
  const imagen =
    publicacion.detalles?.imagenes?.[0] || "/imgs/default-product.svg";

  const contraparteNombre =
    contraparte?.nombre ||
    (esCompra
      ? "Vendedor"
      : `ID: ${trans.comprador_itson_id || "Desconocido"}`);
    ...
    publicacion.titulo || "Producto"
                ...
                <div class="trans-body">
                    <div class="flex items-start justify-between gap-4">
                        <div class="trans-content">
                            <div class="flex items-center gap-2 mb-2">
                                <span class="badge ${
                                  esCompra ? "badge-primary" : "badge-accent"
                                }">
                                    ${esCompra ? "Compra" : "Venta"}
                                </span>
                                <span class="badge ${
                                  estadoClasses[trans.estado] ||
                                  "badge-secondary"
                                }">
                                    ${
                        ...
                        <div class="text-right">
                            <p class="trans-price">
                                ${formatCurrency(
                                  trans.monto || publicacion.precio || 0
                                )}
                            </p>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center gap-3 mt-4 pt-4 trans-divider">
                        <img src="${
                          contraparte?.foto || "/imgs/default-avatar.svg"
                        }" alt="" 
...
/**
 * Cambia el estado de una transacción a "aceptada".
 * @async
...
/**
 * Cambia el estado de una transacción a "cancelada", pidiendo confirmación.
 * @async
...
/**
 * Cambia el estado de una transacción a "completada".
 * @async
 * @function completarTransaccion
 * @param {string} id - ID de la transacción.
 * @returns {Promise<void>}
 */
...
/**
 * Inicializa el sistema de calificación (estrella seleccionada, hover, textos y botón enviar).
...
/**
 * Abre el modal para calificar una transacción, reiniciando estados e inputs.
 * @function abrirModalCalificar
 * @param {string} transaccionId - ID de la transacción a calificar.
 * @returns {void}
 */
...
/**
 * Envía la calificación y comentario al backend.
 * Valida selección mínima y muestra mensajes de resultado.
 * @async
...
async function enviarCalificacion() {
  ...
  try {
    ...
    if (!res.ok) {
      throw new Error(
        data.message || data.error || "Error al enviar calificación"
      );
    }
...
/**
 * Cierra el modal de detalles de transacción.
